...
class IconEditorApp(webserver.App):
    ...
    def show(self, request, response, pathmatch):
        ...
        """Tools List"""
        tool_list = os.listdir("data/tools") #load from toolsfolder
        tools_html = "<ul>"
        for tool_title in tool_list:
            if tool_title != '.DS_Store': #unwelcome macOS file
                with open("data/tools/" + tool_title, "r") as g: #for every file in the folder make icon
                    tools_html += "<li class=tool-list-item><img src='%s' title='%s' id='tool-%s' border='0'></li>" % (g.read(), tool_title,tool_title)
        tools_html += "</ul>" #finish tools list
        """End Toolslist"""
        ...
        for icon_title in icon_list:
            #print(icon_title+"\n")
            if icon_title != 'tools' and icon_title != '.DS_Store': #unwelcome macOS file and not tools folder
        ...
        # returns request with icons and tools
